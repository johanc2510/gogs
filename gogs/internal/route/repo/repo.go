...
func CreatePost(c *context.Context, f form.CreateRepo) {
	...
	if !f.NoRegistration && f.ItemCode != "" {
		
		// Lógica para porcurar nome do item pelo codigo no effective

		repoName = fmt.Sprintf("Repo_%s", f.ItemCode)

	} else if f.NoRegistration {
		// Se o usuário marcou que não possui cadastro, use o nome inserido
		if f.RepoName == "" {
			c.RenderWithErr("Você deve fornecer um nome para o repositório.", CREATE, &f)
			return
		}
		repoName = f.RepoName
	} else {
		// Se nenhum dos casos acima for atendido, retorne um erro
		c.RenderWithErr("Você deve fornecer o código do item ou marcar 'Não possui cadastro no Effective'.", CREATE, &f)
		return
	}
