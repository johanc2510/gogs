...
func FetchRepositories(c *context.Context, page int) ([]map[string]interface{}, error) {
	keyword := c.Query("q")
	if keyword == "" {
		keyword, _ = c.Session.Get("keyword").(string)
	}

	repos, _, err := database.SearchRepositoryByName(&database.SearchRepoOptions{
		Keyword:  keyword,
		UserID:   c.UserID(),
	...
	for _, repo := range repos {
		owner, err := database.GetUserByID(repo.OwnerID)
		if err == nil && owner != nil {
			// Converte o timestamp `UpdatedUnix` para um `time.Time` e formata usando `timeSince`
			updatedTime := time.Unix(repo.UpdatedUnix, 0)
			formattedUpdated := timeSince(updatedTime) // usa a função timeSince para formatar

			repoMap := map[string]interface{}{
				"id":               repo.ID,          // Alterado para minúsculas
				"owner_name":       owner.Name,       // Nome do dono do repositório
				"name":             repo.Name,        // Nome do repositório
				"description":      repo.Description, // Descrição do repositório
				"updated_time_ago": formattedUpdated, // Tempo formatado
				"website":          repo.Website,     // Website do repositório
			}
			repoList = append(repoList, repoMap)
		}
	}
...
func Home(c *context.Context) {
	...
	repos, err := FetchRepositories(c, page)
	if err != nil {
		c.Error(err, "erro ao buscar repositórios")
		return
	}

	if len(repos) == 0 {
		c.Flash.Info("Nenhum repositório encontrado para a palavra-chave.")
	} else {
		c.Data["Repos"] = repos
	}
	...
	// Serializa a lista de repositórios como JSON para o template
	reposJSON, err := json.Marshal(repos)
	if err != nil {
		c.Error(err, "erro ao serializar repositórios")
		return
	}
	c.Data["ReposJSON"] = gotemplate.JS(reposJSON) // Escapamento correto
